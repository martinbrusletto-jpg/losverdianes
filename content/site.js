window.LOS_VERDIALES = {
  heroPoster: '/assets/photos/02-kitchen-living.jpg',
  heroVideo: '/assets/video/kitchen.mp4',
  photos: [
    { src: '/assets/photos/01-bedroom.png', label: 'Hovedsoverom' },
    { src: '/assets/photos/02-kitchen-living.jpg', label: 'Kjøkken & stue' },
    { src: '/assets/photos/03-bathroom.png', label: 'Bad' },
    { src: '/assets/photos/04-lounge.png', label: 'Lounge' },
    { src: '/assets/photos/05-pool-gym.png', label: 'Basseng & gym' },
    { src: '/assets/photos/06-dusk-living.png', label: 'Skumring' }
  ],
  exterior: []
};
